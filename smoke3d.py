...
class SmokeSolver:
    def __init__(self, x, y, z):
        ...
        self._velocities = ti.Vector.field(3, dtype=ti.f32, shape=self.res)
        self._new_velocities = ti.Vector.field(3, dtype=ti.f32, shape=self.res)
        self._vel_temp = ti.Vector.field(3, dtype=ti.f32, shape=self.res)
        self.velocity_divs = ti.field(dtype=ti.f32, shape=self.res)
        self._pressures = ti.field(dtype=ti.f32, shape=self.res)
        self._new_pressures = ti.field(dtype=ti.f32, shape=self.res)
        self._dens_buffer = ti.field(dtype=ti.f32, shape=self.res)
        self._new_dens_buffer = ti.field(dtype=ti.f32, shape=self.res)
        ...
        self._pos = ti.Vector.field(3, dtype=ti.f32, shape=self.res)
        ...
        self.r = [ti.field(dtype=ti.f32) for _ in range(self.n_mg_levels)]  # residual
        ...
        self.alpha = ti.field(dtype=ti.f32)  # step size
        self.beta = ti.field(dtype=ti.f32)  # step size
        self.sum = ti.field(dtype=ti.f32)  # storage for reductions
